...
export const useDiseaseDetection = () => {
  ...
  const predictDisease = useCallback(async (imageFile, cropType) => {
    setLoading(true);
    setError(null);
    setResult(null);
    ...
    try {
      ...
      if (API_ENDPOINT.includes('render') || API_ENDPOINT.includes('railway') || API_ENDPOINT.includes('run.app')) {
...
export const DiseaseDetectionComponent = () => {
  const { predictDisease, loading, error, result, clearError } = useDiseaseDetection();
        ...
        <div className="file-upload">
          ...
          {preview && (
            <div className="image-preview">
              <img src={preview} alt="Selected plant" style={{ maxWidth: '300px', maxHeight: '300px' }} />
            </div>
          )}
        </div>
        ...
        {result && result.success && (
          <div className="results">
            ...
            <div className="result-card">
              <p><strong>Disease:</strong> {result.prediction.disease}</p>
              <p><strong>Confidence:</strong> {(result.prediction.confidence * 100).toFixed(1)}%</p>
              <p><strong>Crop:</strong> {result.prediction.crop_type}</p>
              <p><strong>Status:</strong> {result.prediction.is_healthy ? '🟢 Healthy' : '🔴 Disease Detected'}</p>
            </div>
          </div>
        )}
      </div>
