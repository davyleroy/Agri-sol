...
class TFLiteService extends ChangeNotifier {
  ...
  Future<void> loadModel() async {
    try {
      ...
      final modelData =
          await rootBundle.load('assets/models/agrisol_model.tflite');
      ...
      // Load labels
      final labelsData =
          await rootBundle.loadString('assets/models/model_metadata.txt');
  ...
  Map<String, dynamic> _processResults(List<double> outputBuffer) {
    ...
    if (_labels != null && maxIndex < _labels!.length) {
      ...
      if (detectedCondition.toLowerCase().contains('healthy')) {
        recommendation =
            'Your crop appears healthy. Continue with regular maintenance.';
      } else if (detectedCondition.toLowerCase().contains('blight')) {
        recommendation =
            'Potential blight detected. Consider copper-based fungicides and ensure proper spacing for air circulation.';
      } else if (detectedCondition.toLowerCase().contains('rust')) {
        recommendation =
            'Rust disease detected. Remove affected leaves and apply appropriate fungicide.';
      } else if (detectedCondition.toLowerCase().contains('spot')) {
        recommendation =
            'Leaf spot disease detected. Avoid overhead watering and apply recommended fungicide.';
      } else if (detectedCondition.toLowerCase().contains('mildew')) {
        recommendation =
            'Mildew detected. Improve air circulation and consider sulfur-based treatments.';
      } else {
        recommendation =
            'Issue detected. Consult with local agricultural extension for specific treatment options.';
      }
    }
