-- 🔧 STEP 5: Helper Functions for Location Analytics and Leaderboards
-- This creates all the query functions your frontend will need
    ...
    SELECT 
        ...
        CASE 
            WHEN la.total_scans > 0 THEN 
                ROUND((la.diseased_scans::NUMERIC / la.total_scans::NUMERIC) * 100, 2)
            ELSE 0
        END as disease_rate
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(SUM(la.diseased_scans), 0) as total_diseased_scans,
        COALESCE(SUM(la.unique_users), 0) as unique_users,
...
-- 🦠 Get disease tracking by location
...
BEGIN
    ...
    WHERE 
        (location_filter IS NULL OR dt.location_hierarchy ILIKE '%' || location_filter || '%')
...
BEGIN
    ...
    SELECT 
        ROW_NUMBER() OVER (
            ORDER BY 
                CASE 
                    ...
                    WHEN ranking_type = 'users' THEN la.unique_users::NUMERIC
                    WHEN ranking_type = 'disease_rate' THEN 
                        CASE WHEN la.total_scans > 0 THEN (la.diseased_scans::NUMERIC / la.total_scans::NUMERIC) * 100 ELSE 0 END
        ...
        CASE 
            ...
            WHEN ranking_type = 'users' THEN la.unique_users::NUMERIC
            WHEN ranking_type = 'disease_rate' THEN 
                CASE WHEN la.total_scans > 0 THEN ROUND((la.diseased_scans::NUMERIC / la.total_scans::NUMERIC) * 100, 2) ELSE 0 END
            ELSE la.total_scans::NUMERIC
        END as metric_value,
        CASE 
...
-- 🔍 Search locations by name or hierarchy
...
BEGIN
    ...
    WHERE 
        ...
        OR la.location_hierarchy ILIKE '%' || search_term || '%'
    ORDER BY la.total_scans DESC;
END;
$$ LANGUAGE plpgsql;
...
SELECT 'Testing get_disease_tracking_by_location...' as test_status;
SELECT * FROM get_disease_tracking_by_location() LIMIT 3;
